...
## О воркшопе
В ходе воркшопа мы освоим основные принципы работы "клиент-серверных" web-приложений типа "CRUD" на примере создания простой версии блоговой CMS, а так же поработаем с публичными API на примере социальной сети ВКонтакте.
...
В этой директории находятся 2 файла и множество директорий. Файл `index.js` отвечает за запуск сервера и инициализацию middleware. В файле `helpers.js` расположены некоторые вспомогательные утилитарные методы.    
...
```
Функция-контроллер представляет собой
